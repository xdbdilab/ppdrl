...
class Calattr(object):
    ...
    def GetServiceMap(self):
        files=[]
        file=[]
        filePath ="./julei_result"
        self.getAllFiles(filePath, files, file)
        try:
            for i in range(233):
                br = open(files[i])
               # print("The file:"+str(files))
    ...
    def SetBackUpService(self):
        ...
        for i in range(self.serviceNum):
            ...
            temp.RT = 0
            if len(self.serviceVector[i].parents)==0:
                temp.parents=[]
            else:
    ...
    def GenSet(self):
        ...
        for i in range(1,NODE_SET+1):
            tempset = Set()
            tempset.id = i
            #print("responseTime"+str(i))
            #print(self.mServiceMap)
            #print(self.NodeVector[self.mServiceMap])
    ...
    def Init(self,config,state,num):
    ...
    def GetServiceNode(self,a):
        self.serviceNode=[]
        #print('serviceNum:'+str(self.serviceNum) +'a_length:'+str(len(a)))
        for i in range(self.serviceNum):
            # print('检查出错点：：：：：：：：：：：：：：：：：：：：：：：：：')
            # print(self.mServiceMap[self.nodeSet[0]][0])
            # print(a[0])
    ...
    def init(self,state,num,NODE_NUM): #"conf.xml"

        #需要修改当前节点数
        path="test/"+str(NODE_NUM)+"/%d/conf.xml"%num
        self.Init(path, state, num)
...
if __name__ == "__main__":
    calattr = Calattr()
    calattr.init( "test", 1)
    mArray = [ 2,1,7,0,1,8,0,1,23,1,1,16,0,68,2,3,4,5,13,17,22,6,0, 8,2,6,6,32,0,0,2,1,7,0,1,8,0,1,23,1, 1,16,0,68,2,3,4,5,13,17,0.23933584]
