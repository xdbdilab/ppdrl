...
class Set(object): #233个抽象原子服务的最大最小
    def __init__(self):
        self.id=0
        self.maxTime=0
        self.minTime=0
...
class Calattr(object):
    ...
    def SetNodeSet(self,state):
        self.nodeSet=[]
        #print("!!!!!!!")
        try:
            reader2 = open(state+"\\nodeSet.txt")
            tempString="#{}".format(config.node_num)
    ...
    def Init(self,config,state): #def Init(self,config,state,num):
        self.ReadXMLToGenService(config) #conf.xml
        self.GenService()#抽象原子服务
        self.GetServiceRelation(config)#father child
        self.GetServiceMap()# 抽象原子服务对应的候选服务集
        self.GenNode()#得到所有的原子服务
        self.GetPrice()#计算所有原子服务的价格
        self.SetBackUpService()#备份所有的抽象原子服务
        self.GenSet()#计算233个抽象原子服务的max min

        self.SetNodeSet(state) #get nodeset.txt

        self.GetGraph()#parent==>children 1
        self.GetConnection() #是否可达
    ...
    def obj_eval(self,x_var):
        ...
        return y_obj
    def init(self,conf,state):
        self.Init(conf, state)  # self.Init("conf.xml",state,num)
        #计算组合服务的max min，用于归一化
